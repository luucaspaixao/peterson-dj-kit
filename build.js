...
function generateSocialLinksHTML(links) {
	...
	return Object.entries(links)
		...
		.map(
			([key, url]) =>
				`<a class="text-white/40 hover:text-brand transition-colors" href="${url}" target="_blank" rel="noopener">${names[key] || key.toUpperCase()}</a>`,
		)
		.join("\n");
}
...
html = html.replace(
	/\{\{site\.hero_cta_primary\}\}/g,
	content.site.hero_cta_primary,
);
html = html.replace(
	/\{\{site\.hero_cta_secondary\}\}/g,
	content.site.hero_cta_secondary,
);
html = html.replace(/\{\{site\.bpm_display\}\}/g, content.site.bpm_display);

// About
html = html.replace(
	/\{\{about\.section_title\}\}/g,
	content.about.section_title,
);
html = html.replace(/\{\{about\.photo\}\}/g, content.about.photo);
html = html.replace(
	/\{\{about\.bio_paragraph_1\}\}/g,
	content.about.bio_paragraph_1,
);
html = html.replace(
	/\{\{about\.bio_paragraph_2\}\}/g,
	content.about.bio_paragraph_2,
);
html = html.replace(
	/\{\{about\.genres\}\}/g,
	generateGenresHTML(content.about.genres),
);

// Dynamic sections
html = html.replace(
	/\{\{releases\}\}/g,
	generateReleasesHTML(content.releases),
);
...
// Press Kit
html = html.replace(
	/\{\{press_kit\.description\}\}/g,
	content.press_kit.description,
);
html = html.replace(
	/\{\{press_kit\.epk_file\}\}/g,
	content.press_kit.epk_file || "#",
);
html = html.replace(
	/\{\{press_kit\.tech_rider_file\}\}/g,
	content.press_kit.tech_rider_file || "#",
);
html = html.replace(
	/\{\{press_kit\.video_reel_file\}\}/g,
	content.press_kit.video_reel_file || "#",
);
...
html = html.replace(
	/\{\{social_links\}\}/g,
	generateSocialLinksHTML(content.social_links),
);
...
html = html.replace(
	/\{\{page_title\}\}/g,
	`${content.site.title} // Digital Press Kit`,
);
...
// Copia TODA a pasta public/ para dist/ (imagens, admin, downloads, etc.)
...
copyDirSync(path.join(__dirname, "public"), distDir);
...
console.log(
	`   Seções: Hero, About, ${content.releases.length} Releases, ${content.gallery.length} Fotos, ${content.videos.length} Vídeos, Press Kit, Contato`,
);
